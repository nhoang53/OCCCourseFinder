...
class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase database) {
        ...
        createQuery = "CREATE TABLE " + OFFERINGS_TABLE + "("
                + OFFERINGS_KEY_FIELD_ID + " INTEGER PRIMARY KEY, " // don't need autoincrement
    ...
    public void addOffering(int crn, int semesterCode, int courseId, int instructorId) { // (Offering offering)
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
    ...
    public boolean importOfferingFromCSV(String csvFileName) {
        ...
        try {
            while ((line = buffer.readLine()) != null) {
                ...
                int crn = Integer.parseInt(fields[0].trim()); // clear space at the end
                int semester_id = Integer.parseInt(fields[1].trim());
                int course_id = Integer.parseInt(fields[2].trim());
                int instructor_id = Integer.parseInt(fields[3].trim());
                addOffering(crn, semester_id, course_id, instructor_id);
            }
        } catch (IOException e) {
            e.printStackTrace();
            return false;
